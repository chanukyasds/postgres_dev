...
TupleDesc get_tupledesc(int elementType)
{
    ...
    for (i = 0; i < total_rows; i++)
    {
        ...
        attnum = SPI_getbinval(tuple, 1, &isnull1);
        attname = SPI_getvalue(tuple, 2);
        oidtypeid = SPI_getbinval(tuple, 3, &isnull2);
        typmod = SPI_getbinval(tuple, 4, &isnull3);
        attdim = SPI_getbinval(tuple, 5, &isnull4);
...
Datum split_array(PG_FUNCTION_ARGS)
{
    ...
    int elementType = ARR_ELEMTYPE(input_array);
    int elementWidth;
    ...
    {
        fields = palloc(sizeof(Datum *)*funcctx->max_calls);
